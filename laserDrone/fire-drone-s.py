...
def main():
    global laserOverlay,explOverlay,camera,strike,vivant,board
    laserShot=None
    board = MultiWii()
    board.start('/dev/ttyUSB0')
...
def remote_callback(code):
    global hit,laserOverlay,vivant,board
    print('\r\r-----YOU GOT HIT---')  # unknown code
    strike=strike+1
    print "Strike #:",strike,"\r"
    #get actual values from RC
    rc_values = board.getData(MultiWii.RC)
    data = [1800,rc_values['pitch'],rc_values['yaw'],rc_values['throttle'] ]
    start = time.time()
