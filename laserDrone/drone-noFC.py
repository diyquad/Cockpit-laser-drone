...
def main():
    ...
    strip.begin()
    colorWipe(strip, Color(255, 255, 255), 0)  # white wipe
    print "led ok"
    laserShot=None
    """Connexion to drone"""
    ...
    with picamera.PiCamera() as camera:
       ...
       irRxPin = 18
       GPIO.setup(irRxPin, GPIO.OUT)
       GPIO.output(irRxPin, GPIO.LOW)
       time.sleep(0.1)
       #Change the pin back to input
       GPIO.setup(irRxPin, GPIO.IN)
       ''' GPIO.setmode(GPIO.BCM)
       GPIO.setup(irRxPin,GPIO.IN)'''
       GPIO.add_event_detect(irRxPin,GPIO.BOTH,callback=drone_Hit)
       """Connection du IR TX"""
       irTxPin = 15
       GPIO.setmode(GPIO.BCM)
